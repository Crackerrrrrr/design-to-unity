...
mcp = FastMCP("FunPlayLanhuGameMcp")
...
async def lanhu_design_get_handoff_profile(
    ...
    return {
        ...
        "profile": profile,
        "handoff_reminder": [
            "This MCP does not create Unity prefabs directly.",
...
def main() -> None:
    ...
    print(f"FunPlayLanhuGameMcp listening on http://{settings.server_host}:{settings.server_port}/mcp")
    print("Client config example:")
    print('{"mcpServers":{"FunPlayLanhuGameMcp":{"url":"http://localhost:%d/mcp"}}}' % settings.server_port)
