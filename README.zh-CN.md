...
针对多设计源工作流，它可以读取蓝湖项目和设计页、Figma 文件和插件导出物、PSD / PSB 分层文件，以及 Photoshop UXP 导出目录，保留布局、坐标、文本、切图、组件提示、视觉效果、复用关系和风险信息，再输出给 Unity MCP、静态 prefab YAML 或 Unity Editor importer 继续落地。

它的核心目标不是替代 Unity 编辑器，而是给 AI 和 Unity MCP 提供足够完整、低歧义的设计信息，让设计稿可以更稳定地复原为可检查、可继续加工的 Unity UI。

## 文档和使用路径

- 想快速试用：先看本文的“快速开始”和各设计源工具列表。
- 想接入 Photoshop 或 Figma 导出器：看 `templates/photoshop-uxp-exporter/README.md` 和 `templates/figma-plugin-exporter/README.md`。
- 想了解发布和仓库维护：看 `docs/publishing.md` 和 `docs/launch-kit.md`。
- 内部架构方案、测试记录和生成物不属于产品发布内容，保留在开发空间。
...
| Figma | REST 文件、Frame、Component、插件导出、Auto Layout、constraints、图片填充、variants、variables、prototype reactions 和复杂视觉风险。 |
...
这条链路不是简单截图还原，而是把不同设计来源都转成 AI 和 Unity 能理解的实现计划：

- 在线模式读取蓝湖和 Figma 设计源，包括项目页、设计页、Frame、组件、切图、样式、图片填充和导出地址。
- 本地模式读取 Figma snapshot、PSD / PSB 文件、Photoshop UXP 导出物和插件导出的设计目录，用于离线回归和批量验证。
- 导出器模式可以捕获选中内容、预览 PNG、人工语义标记、文本元数据，以及需要切图兜底的复杂 Vector / 图片资源。
- Packet 会保留 `render_strategy`、`render_rect`、`visual_bounds`、`source_semantics`、来源专属布局提示、文本元数据和 reusable prefab 候选。
...
Design to Unity 的文本策略是 **可编辑优先**：Figma / PSD / 蓝湖文本默认生成可编辑 `TextMeshProUGUI`，不会因为缺少 TMP 字体就自动回退为文字切图。缺少字体时，工具会在 writer result、source map 和 readiness report 中反馈给用户，由用户补齐 TMP Font Asset 映射后再做最终视觉验收。
...
readiness report 会输出 `text_restoration_policy`、`font_requirements`、`missing_tmp_font_mapping_count` 和缺失字体样例。只有用户显式设置 `use_text_components=false` 时，文本才会按图片切图输出。
